...
int main() {
  ...
  // Start the from center lane
  int lane = 1;

  // Minimum number of cycles before attempting a new lane change
  int keep_lane = 50;

  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy, &lane, &keep_lane, &help, &pathplanner](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
